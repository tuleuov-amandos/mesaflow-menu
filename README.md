...
- **Первый экран — «Популярное»**: до 8 хитов крупно, заказ любимого в один тап;
  категории и полный каталог — вторичны, ниже
- Поиск в реальном времени (с нормализацией регистра) и избранное
- **Повтор прошлого заказа** в один тап: снимок последнего заказа кладётся в корзину
- **Память о клиенте**: имя, телефон и предпочтения (получение/оплата) подставляются
  автоматически — постоянный клиент только подтверждает заказ
- **Живое отслеживание заказа**: карточка со степпером «Принят → Готовится → Готов → Выдан»
  и ETA, статус подтягивается из API коротким опросом (клиент видит «Готов, забирайте»)
...
- Самовывоз (по умолчанию) или доставка, с условным полем адреса
...
- Публичное отслеживание заказа идёт по техническому коду `MF-XXXXXX` как по «токену»
  (аналог номера посылки): наружу отдаются только статус и ETA, без персональных данных.
